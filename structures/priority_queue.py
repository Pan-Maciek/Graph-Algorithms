...
def min_priority(a, b):
    return a < b

def swap(array, a, b):
    array[a], array[b] = array[b], array[a]

def heapify(array, i, N, cmp=max_priority, swap=swap):
    while True:
        L, R = 2 * i + 1, 2 * i + 2
        S = i  # selected
...
def build_heap(array, i, N, cmp=max):
    for i in range(N // 2, -1, -1):
        heapify(array, i, N, cmp=cmp)

def sift_up(array, i, cmp=max, swap=swap):
...
class priority_queue(object):
    ...
    def __init__(self, max_size, cmp=max):
        ...
        self._raw_cmp = cmp
        def _cmp(a, b):
            return cmp(self._info[a].priority, self._info[b].priority)
        self._cmp = _cmp
        def swap(keys, a, b):
    ...
    def _push(self, key, priority):  # skip uniq key check
        ...
        sift_up(self._keys, self.size, cmp=self._cmp, swap=self._swap)
        self.size += 1
    ...
    def pop(self):
        ...
        if self.size != 0:
            ...
            heapify(self._keys, 0, self.size, self._cmp, swap=self._swap)
        return (key, priority)

    def change_priority(self, key, priority):
        old_priority = self._info[key].priority
        self._info[key].priority = priority
        if self._raw_cmp(priority, old_priority): #?
            sift_up(self._keys, self._info[key].index, cmp=self._cmp, swap=self._swap)
        else:
            heapify(self._keys, self._info[key].index, self.size, cmp=self._cmp, swap=self._swap)
